...
df = df[df[income_column] > 0]

# P90/P10 ratio with outlier handling
def calculate_p90_p10_ratio_with_ci(group):
...
p90_p10_results = df.groupby(generation_column).apply(calculate_p90_p10_ratio_with_ci)
generation_order = ['Baby Boomers', 'Generation X', 'Millennials', 'Generation Z']
p90_p10_results = p90_p10_results.reindex(generation_order).dropna()

categories = p90_p10_results.index.tolist()
p90_p10_values = [result[0] for result in p90_p10_results]
error_bars = [result[1] for result in p90_p10_results]

x = np.arange(len(categories))
bar_width = 0.4

plt.figure(figsize=(10, 6))
plt.bar(x, p90_p10_values, bar_width, color="purple", alpha=0.8, yerr=error_bars, capsize=5, error_kw=dict(ecolor="black", elinewidth=1.5))
for i in range(len(x)):
    plt.text(x[i], p90_p10_values[i] + error_bars[i] + 0.2, f"{p90_p10_values[i]:.1f}", ha='center', fontsize=10)
plt.title("P90/P10 Ratio by Generation", fontsize=16)
